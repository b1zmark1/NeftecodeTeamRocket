...
PROP_UNIT = "Единица измерения_по_партиям"
...
def detect_sep(path: Path) -> str:
    """
    Detect CSV delimiter using a short sample.
...
def normalize_batch(value: object) -> str:
    """
    Normalize batch values to reduce merge failures.
    Keep 'typical' as-is because it has semantic meaning.
    """
...
def safe_share_true(series: pd.Series) -> float:
    if len(series) == 0:
        return np.nan
...
def enrich_mixtures(
    ...
    # -----------------------------
    # Готовим безопасные копии property tables
    # -----------------------------
    exact = properties_exact.copy()
    typical = properties_typical.copy()
    ...
    property_cols = [
        col for col in properties_exact.columns
        if col not in {COL_COMPONENT, COL_BATCH}
    ]
    ...
    flag_df = pd.DataFrame(flag_data, index=merged.index)
    merged = pd.concat([merged, flag_df], axis=1)

    # -----------------------------
    # Удаляем только технические колонки из property merges
    # -----------------------------
    drop_cols = [c for c in ["_prop_component", "_prop_batch"] if c in merged.columns]
    # после второго merge pandas может назвать правый ключ _prop_component_typical_right или оставить как есть
    ...
    # -----------------------------
    # Компонентные служебные признаки
    # -----------------------------
    if COL_COMPONENT not in merged.columns:
        raise KeyError(
            f"После merge потеряна колонка {COL_COMPONENT!r}. "
            f"Текущие колонки: {list(merged.columns)}"
        )
...
def main() -> None:
    ...
    # Feature matrix
    feature_cols = [
        col for col in train_scenarios.columns
        if col not in {COL_SCENARIO, TARGET_VISC, TARGET_OXID}
        and pd.api.types.is_numeric_dtype(train_scenarios[col])
    ]
    ...
    X_test = test_scenarios[feature_cols].copy()
